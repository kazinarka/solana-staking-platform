...
fn main() {
    ...
    if let Some(matches) = matches.subcommand_matches("stake3") {
        let url = match matches.value_of("env") {
            Some("dev") => "https://api.devnet.solana.com",
            _ => "https://api.mainnet-beta.solana.com",
        };
        ...
        let nft1 = matches.value_of("nft1").unwrap().parse::<Pubkey>().unwrap();
        let nft2 = matches.value_of("nft2").unwrap().parse::<Pubkey>().unwrap();
        let nft3 = matches.value_of("nft3").unwrap().parse::<Pubkey>().unwrap();

        let (metadata1, _) = Pubkey::find_program_address(
            &[
                "metadata".as_bytes(),
                &spl_token_metadata::ID.to_bytes(),
                &nft1.to_bytes(),
        ...
        let (metadata2, _) = Pubkey::find_program_address(
            &[
                "metadata".as_bytes(),
                &spl_token_metadata::ID.to_bytes(),
                &nft2.to_bytes(),
        ...
        let (metadata3, _) = Pubkey::find_program_address(
            &[
                "metadata".as_bytes(),
                &spl_token_metadata::ID.to_bytes(),
                &nft3.to_bytes(),
        ...
        let source1 =
            spl_associated_token_account::get_associated_token_address(&wallet_pubkey, &nft1);
        let source2 =
            spl_associated_token_account::get_associated_token_address(&wallet_pubkey, &nft2);
        let source3 =
            spl_associated_token_account::get_associated_token_address(&wallet_pubkey, &nft3);

        let destanation1 =
            spl_associated_token_account::get_associated_token_address(&vault, &nft1);
        let destanation2 =
            spl_associated_token_account::get_associated_token_address(&vault, &nft2);
        let destanation3 =
            spl_associated_token_account::get_associated_token_address(&vault, &nft3);

        let (stake_data1, _) = Pubkey::find_program_address(&[&nft1.to_bytes()], &program_id);
        let (stake_data2, _) = Pubkey::find_program_address(&[&nft2.to_bytes()], &program_id);
        let (stake_data3, _) = Pubkey::find_program_address(&[&nft3.to_bytes()], &program_id);

        let metadata_data1 = client.get_account_data(&metadata1).unwrap();
        let metadata_data2 = client.get_account_data(&metadata2).unwrap();
        let metadata_data3 = client.get_account_data(&metadata3).unwrap();

        let metadata_data_struct1: spl_token_metadata::state::Metadata =
            try_from_slice_unchecked(&metadata_data1[..]).unwrap();
        let metadata_data_struct2: spl_token_metadata::state::Metadata =
            try_from_slice_unchecked(&metadata_data2[..]).unwrap();
        let metadata_data_struct3: spl_token_metadata::state::Metadata =
            try_from_slice_unchecked(&metadata_data3[..]).unwrap();

        let creator1 = metadata_data_struct1
            .data
            .creators
            .unwrap()
            .first()
            .unwrap()
            .address;
        let creator2 = metadata_data_struct2
            .data
            .creators
            .unwrap()
            .first()
            .unwrap()
            .address;
        let creator3 = metadata_data_struct3
            .data
            .creators
            .unwrap()
            .first()
        ...
        let (wl_data_address1, _wl_data_address_bump) = Pubkey::find_program_address(
            &["whitelist".as_bytes(), &creator1.to_bytes()],
            &program_id,
        );
        let (wl_data_address2, _wl_data_address_bump) = Pubkey::find_program_address(
            &["whitelist".as_bytes(), &creator2.to_bytes()],
            &program_id,
        );
        let (wl_data_address3, _wl_data_address_bump) = Pubkey::find_program_address(
            &["whitelist".as_bytes(), &creator3.to_bytes()],
            &program_id,
        );

        let instractions = vec![Instruction::new_with_borsh(
            program_id,
            &PlatformInstruction::Stake3,
            vec![
                AccountMeta::new(wallet_pubkey, true),
                AccountMeta::new_readonly(nft1, false),
                AccountMeta::new_readonly(nft2, false),
                AccountMeta::new_readonly(nft3, false),
                AccountMeta::new_readonly(metadata1, false),
                AccountMeta::new_readonly(metadata2, false),
                AccountMeta::new_readonly(metadata3, false),
                AccountMeta::new_readonly(vault, false),
                AccountMeta::new(source1, false),
                AccountMeta::new(source2, false),
                AccountMeta::new(source3, false),
                AccountMeta::new(destanation1, false),
                AccountMeta::new(destanation2, false),
                AccountMeta::new(destanation3, false),
                AccountMeta::new_readonly(spl_token::id(), false),
                AccountMeta::new_readonly(system_program::id(), false),
                AccountMeta::new_readonly(
                    "SysvarRent111111111111111111111111111111111"
                ...
                AccountMeta::new(stake_data1, false),
                AccountMeta::new(stake_data2, false),
                AccountMeta::new(stake_data3, false),
                AccountMeta::new(wl_data_address1, false),
                AccountMeta::new(wl_data_address2, false),
                AccountMeta::new(wl_data_address3, false),
            ],
        )];

        let mut tx = Transaction::new_with_payer(&instractions, Some(&wallet_pubkey));
        let recent_blockhash = client.get_latest_blockhash().expect("Can't get blockhash");
        tx.sign(&vec![&wallet_keypair], recent_blockhash);
        let id = client.send_transaction(&tx).expect("Transaction failed.");
        println!("tx id: {:?}", id);
